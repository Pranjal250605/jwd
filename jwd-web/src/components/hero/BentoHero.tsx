...
function StatCell({
  ...
  return (
    <Cell delay={delay} reduce={reduce} className="flex flex-col justify-between gap-6 transition-colors duration-500 hover:bg-washi-deep">
      <span className="text-sm uppercase tracking-[0.28em] text-gold">{kicker}</span>
      <div ref={ref} className="flex flex-col gap-2">
        ...
        {note && (
          <span className="font-mono text-sm uppercase tracking-[0.16em] text-sumi-soft/70">
            {note}
          </span>
        )}
      </div>
  ...
  return (
    <section className="relative overflow-hidden bg-washi pt-28 pb-16 lg:pt-32 lg:pb-20">
      ...
      <div
        ...
        style={{ background: 'radial-gradient(ellipse at 80% 90%, rgba(0,151,167,0.08) 0%, transparent 65%)' }}
      />
        ...
        >
          ...
          >
            ...
            <div className="relative">
              <div className="mb-8 flex items-center gap-4">
                ...
                />
                <span className="text-sm uppercase tracking-[0.4em] text-gold">{t('badge')}</span>
              </div>
            ...
            <div className="relative flex flex-col gap-7">
              ...
              <div className="flex flex-wrap items-center gap-4">
                <Link
                  href="/contact"
                  className="group/btn relative overflow-hidden bg-sumi px-8 py-4 text-sm uppercase tracking-[0.24em] text-washi"
                >
                  <span
                ...
                <Link
                  href="/contact"
                  className="border border-gold/35 px-8 py-4 text-sm uppercase tracking-[0.24em] text-sumi transition-colors duration-500 hover:border-gold hover:bg-gold/5"
                >
                  {t('ctaSecondary')}
                </Link>
              </div>
          ...
          <Cell delay={0.1} reduce={reduce} className="flex flex-col justify-between gap-6 transition-colors duration-500 hover:bg-washi-deep">
            <div className="flex items-center justify-between">
              <span className="text-sm uppercase tracking-[0.28em] text-gold">{d('fxLabel')}</span>
              <span className="flex items-center gap-1.5">
                ...
                <span className="font-mono text-[15px] tracking-[0.2em] text-gold/70">{d('live')}</span>
              </span>
            </div>
          ...
          <Cell delay={0.22} reduce={reduce} className="flex flex-col justify-between gap-6 transition-colors duration-500 hover:bg-washi-deep">
            <span className="text-sm uppercase tracking-[0.28em] text-gold">{d('yieldLabel')}</span>
          ...
          <Link href="/heart-of-europe" className="contents">
            <Cell delay={0.34} reduce={reduce} className="flex items-center justify-between gap-6 transition-colors duration-500 hover:bg-washi-deep sm:col-span-2">
              <div className="flex flex-col gap-3">
                <span className="text-sm uppercase tracking-[0.28em] text-gold">
                ...
                <span className="inline-flex items-center gap-2 text-sm uppercase tracking-[0.2em] text-sumi-soft transition-all duration-500 group-hover:gap-3 group-hover:text-gold">
                  {ja ? '詳しく見る' : 'Explore'} <span aria-hidden>→</span>
                </span>
              </div>
              <svg aria-hidden viewBox="0 0 100 100" className="h-24 w-24 shrink-0 opacity-80">
                <circle cx="50" cy="48" r="40" fill="none" stroke="rgba(0,151,167,0.2)" strokeWidth="0.4" strokeDasharray="2 3" />
                {ISLANDS.map(([x, y, r], i) => (
                  <circle key={i} cx={x} cy={y} r={r} fill={i % 3 === 0 ? '#0097a7' : 'rgba(0,151,167,0.4)'} />
                ))}
              </svg>
            </Cell>
          </Link>
          ...
          <Cell delay={0.4} reduce={reduce} className="flex items-center justify-between gap-6 transition-colors duration-500 hover:bg-washi-deep sm:col-span-2">
            <div className="flex flex-col gap-2">
              ...
              <span className="text-sm uppercase tracking-[0.24em] text-sumi-soft/70">
                {ja ? 'クロスボーダー資産設計' : 'Cross-border wealth'}
              </span>
            </div>
            <span
              className="font-jp shrink-0 px-5 py-2 text-[14px] tracking-[0.3em] text-gold"
