...
const ACCENT = '#9a7b2d';
const RX = 380; // horizontal radius of the orbit
const RZ = 320; // depth radius
type Lang = 'ja' | 'en';
...
function OrbitCard({
  ...
  const x = useTransform(rotation, (r) => {
    const theta = ((i / n) * 360 + r) * (Math.PI / 180);
    return Math.sin(theta) * RX;
  });
  const z = useTransform(rotation, (r) => {
    const theta = ((i / n) * 360 + r) * (Math.PI / 180);
    return Math.cos(theta) * RZ;
  });
  ...
  return (
    ...
    >
      <div className="overflow-hidden rounded-3xl border border-white/10 bg-washi shadow-[0_50px_90px_-35px_rgba(12,14,18,0.65)]">
        <div className="relative aspect-[5/4] overflow-hidden">
          ...
          <div
            ...
            style={{ background: 'linear-gradient(to top, rgba(12,14,18,0.82) 0%, rgba(12,14,18,0.2) 42%, transparent 70%)' }}
          />
        ...
        <div className="flex flex-col gap-3 p-5">
          <h3 className="font-jp text-[0.95rem] font-bold leading-snug text-sumi">
...
/** Property cards floating on a rotating 3D ellipse. Auto-spins, drag to explore. */
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0" style={{ transformStyle: 'preserve-3d' }}>
        {listings.map((p, i) => (
          <OrbitCard key={p.id} p={p} i={i} n={n} rotation={rotation} locale={locale} />
        ))}
      </div>
    </div>
  );
