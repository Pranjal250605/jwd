      ...
      >
        ...
        <div className="relative h-60 sm:h-80 lg:h-auto">
          ...
          <div className="absolute bottom-5 left-5 inline-flex items-center gap-2 rounded-full bg-washi/90 px-3.5 py-1.5 backdrop-blur-sm">
            ...
            <span className="text-[9px] uppercase tracking-[0.16em] text-sumi-soft">{ja ? '目標利回り' : 'Target yield'}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-7 overflow-y-auto p-7 lg:p-9">
          <div className="flex flex-col gap-2.5">
            <span className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-[0.24em]" style={{ color: ACCENT }}>
          ...
          <div className="grid grid-cols-2 overflow-hidden rounded-2xl border border-sumi/8">
            <div className="bg-washi-deep/40 p-5">
              <span className="text-[9px] uppercase tracking-[0.2em] text-sumi-soft">{ja ? '販売価格' : 'Asking price'}</span>
            ...
            <div className="border-l border-sumi/8 p-5" style={{ background: `${ACCENT}0c` }}>
              <span className="text-[9px] uppercase tracking-[0.2em] text-sumi-soft">{ja ? '目標利回り' : 'Target yield'}</span>
          ...
          <div className="grid grid-cols-3 gap-3">
            {specs.map(({ Icon, label, value }, i) => (
              <div key={i} className="flex flex-col items-center gap-2 rounded-xl border border-sumi/8 px-2 py-4 text-center">
                ...
                <span className="text-[8px] uppercase tracking-[0.16em] text-sumi-soft">{label}</span>
              </div>
            ))}
          </div>

          <p className="text-[0.92rem] font-light leading-[1.9] text-sumi-soft">{desc}</p>
          ...
          <div className="mt-auto flex flex-col gap-3 border-t border-sumi/8 pt-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <a href={p.bayut} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-1.5 rounded-full border border-sumi/15 px-4 py-3 text-[10px] font-medium uppercase tracking-[0.16em] text-sumi transition-colors hover:border-gold hover:text-gold">
                Bayut <ArrowUpRight className="h-3.5 w-3.5" strokeWidth={1.6} />
              </a>
              <a href={p.pf} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-1.5 rounded-full border border-sumi/15 px-4 py-3 text-[10px] font-medium uppercase tracking-[0.16em] text-sumi transition-colors hover:border-gold hover:text-gold">
                Property Finder <ArrowUpRight className="h-3.5 w-3.5" strokeWidth={1.6} />
              </a>
            </div>
            <p className="text-[10px] font-light italic text-sumi-soft/70">
