    ...
    <section id="properties" className="relative overflow-hidden bg-washi py-28 lg:py-36">
      ...
      <div className="relative z-10 mx-auto max-w-screen-2xl px-7 lg:px-12">
        <Reveal className="mb-14 flex flex-col gap-4 lg:mb-18">
          <span className="text-sm uppercase tracking-[0.38em] text-gold">{t('label')}</span>
            ...
            <Reveal key={p.id} delay={0.1 + i * 0.08}>
              ...
              >
                ...
                <div className="flex flex-1 flex-col gap-2.5 p-5">
                  <span className="text-sm uppercase tracking-[0.28em] text-gold/70">{ja ? p.typeJa : p.typeEn}</span>
                  <h3 className="font-jp text-[0.95rem] font-bold leading-snug text-sumi">{ja ? p.nameJa : p.nameEn}</h3>
                  <div className="flex items-center gap-3.5 text-[15px] text-sumi-soft">
                  ...
                  <div className="mt-auto flex items-center justify-between border-t border-sumi/8 pt-3.5">
                    ...
                    <span className="inline-flex items-center gap-1 text-sm uppercase tracking-[0.16em] text-gold opacity-0 -translate-x-1 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100">
        ...
        <Reveal delay={0.3} className="mt-8">
          <p className="text-sm tracking-[0.1em] text-sumi/55">{t('note')}</p>
        </Reveal>
      </div>
    </section>
  );
